...
// RackframeDrivePulse counts rackframe-side effects from FSM after hooks (acceptance).
var RackframeDrivePulse func()

func RegisterRackframeDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if RackframeDrivePulse != nil {
			RackframeDrivePulse()
		}
		return nil
	})
