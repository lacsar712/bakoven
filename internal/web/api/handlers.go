...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("POST /proof/start", withTimeout(s.handleStartProof))
	mux.HandleFunc("POST /proof/complete", withTimeout(s.handleCompleteProof))
	...
	mux.HandleFunc("GET /tray/level", withTimeout(s.handleTrayLevel))
...
func (s *Server) handleWarmup(w http.ResponseWriter, r *http.Request) {
	...
	writeOK(w, map[string]interface{}{
		"ready":  ready,
		"detail": detail,
		"proof":  s.app.ProofRemaining(),
		"warmup": s.app.BurnerWarmupRemaining(),
	})
}

func (s *Server) handleStartProof(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.StartProof(r.Context(), holder); err != nil {
...
func (s *Server) handleCompleteProof(w http.ResponseWriter, r *http.Request) {
	holder := r.URL.Query().Get("holder")
	if holder == "" {
		holder = "hmi-operator"
	}
	if err := s.app.CompleteProof(r.Context(), holder); err != nil {
		writeErr(w, http.StatusConflict, err)
		return
	}
	writeOK(w, s.app.Snapshot())
...
func (s *Server) handleTrayLevel(w http.ResponseWriter, r *http.Request) {
	if err := s.app.CheckTrayLevel(s.app.Snapshot()); err != nil {
		writeErr(w, http.StatusConflict, fmt.Errorf("ovenline fault: %w", err))
		return
	}
