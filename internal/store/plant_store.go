...
func (s *PlantStore) UpdateTray(unitID string, tray model.TrayReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateOvenline(unitID string, ovenline model.OvenlineReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
